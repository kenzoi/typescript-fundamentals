 ...
 */
const y = "hello world";  // string literal type, y: "hello world" = "hello world";
 ...
 */
let z; // takes type ANY - Top type - accepts any values
z = 41;
 ...
 */
let aa2 = []; // Array of nevers - it can never happen - we can't push anything to it. Bottom type

let aa: number[] = []; // aa = [3] --> This has type number[]
aa.push(33);
...
bb.push(4, 4, 4, 4, 4) // With Tuples we don't should use array methods, doesn't have type check with array methods.

/**
 * (10) Tuple values often require type annotations (  : [number, number] )
 */
const xx = [32, 31]; // number[];
const yy: [number, number] = [32, 31]; // !! To be a tuple we need to set types, otherwise it'll be an array like line above
...
// contactInfo.phone; // 🚨 error, we only can access the property that we are guaranteed to be there
/**
 * (15) Union types
 */
